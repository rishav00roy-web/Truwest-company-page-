...
interface ReviewWallProps {
  ...
  showGoogleButtons?: boolean;
}

export default function ReviewWall({ eyebrow = 'Client Reviews', title, reviews, showGoogleButtons = true }: ReviewWallProps) {
  return (
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-[20px] text-left mb-[56px]">
          {reviews.map((review, idx) => (
            <div key={idx} className="bg-white border border-line p-[30px_28px_26px] mb-[20px] break-inside-avoid">
              <span className="text-bronze text-[13px] tracking-[3px] block mb-[14px]">
